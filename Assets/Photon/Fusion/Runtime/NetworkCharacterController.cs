    ...
    public sealed unsafe class NetworkCharacterController : NetworkTRSP, INetworkTRSPTeleport, IBeforeAllTicks, IAfterAllTicks, IBeforeCopyPreviousState {
        ...
        public void Move(Vector3 direction) {
            ...
            if (direction == default) {
                ...
                var finalDir = isGrounded ? horizontalVel + direction * acceleration * deltaTime : horizontalVel;
                horizontalVel = Vector3.ClampMagnitude(finalDir, maxSpeed);
                transform.rotation = isGrounded ? Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(direction), rotationSpeed * Runner.DeltaTime) : transform.rotation;
            }
